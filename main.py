...
BASE_SERVICES: dict[str, dict[str, Any]] = {
    "likes": {
        "label": "Instagram Likes", "short": "Likes", "unit": "likes", "emoji_key": "HEART", "style": "danger",
        "min": 100, "max": 2_000, "step": 100, "rate_per_1k": 90,
        "api_url": "https://electrosmm.com/api/v2", "api_key": "", "service_id": "", "enabled": True,
    },
    "views_slow": {
        "label": "Instagram Views · Slow", "short": "Slow Views", "unit": "views", "emoji_key": "EYE", "style": "primary",
        "min": 100, "max": 10_000, "step": 100, "rate_per_1k": 3,
        "api_url": "https://luvsmm.com/api/v2", "api_key": "", "service_id": "1137", "enabled": True,
    },
    "views_instant": {
        "label": "Instagram Views · Instant", "short": "Instant Views", "unit": "views", "emoji_key": "ROCKET", "style": "success",
        "min": 100, "max": 10_000, "step": 100, "rate_per_1k": 5,
        "api_url": "https://luvsmm.com/api/v2", "api_key": "", "service_id": "160", "enabled": True,
    },
}
VIEW_KINDS = ("views_slow", "views_instant")

# field code -> (storage key, human label, type)
EDITABLE_FIELDS: dict[str, tuple[str, str, str]] = {
    "label": ("label", "Display name", "str"),
    "url": ("api_url", "API URL", "url"),
    "key": ("api_key", "API key", "secret"),
    "sid": ("service_id", "Service ID", "str"),
    "rate": ("rate_per_1k", "Rate per 1K (coins)", "float"),
    "min": ("min", "Min quantity", "int"),
    "max": ("max", "Max quantity", "int"),
    "step": ("step", "Quantity step", "int"),
}

SERVICES: dict[str, dict[str, Any]] = {k: dict(v) for k, v in BASE_SERVICES.items()}
...
def provider_ready(spec: dict) -> bool:
    return bool(str(spec.get("api_url", "")).strip() and str(spec.get("api_key", "")).strip()
                and str(spec.get("service_id", "")).strip())
...
def check_quantity(kind: str, quantity: int) -> str | None:
    spec = service(kind)
    if quantity < spec["min"] or quantity > spec["max"]:
        return f"Quantity must be between {spec['min']:,} and {spec['max']:,}."
    if spec["step"] > 1 and quantity % spec["step"]:
...
class MongoStore:
    ...
    # ---- orders ----------------------------------------------------------------------
    async def create_order(self, order_id: str, uid: int, kind: str, link: str, qty: int, coins: int) -> dict:
        now = utcnow()
        order = {"order_id": order_id, "user_id": uid, "kind": kind, "link": link, "quantity": qty, "coins": coins,
...
class MemoryStore:
    ...
    async def create_order(self, order_id, uid, kind, link, qty, coins):
        now = utcnow()
        order = {"order_id": order_id, "user_id": uid, "kind": kind, "link": link, "quantity": qty, "coins": coins,
                 "status": "pending", "provider_order_id": None, "provider_error": None, "provider_status": None,
                 "remains": None, "start_count": None, "refunded": 0, "created_at": now, "updated_at": now}
...
async def load_services() -> None:
    ...
    # migrate from the old v1 layout ("provider.<kind>")
    for kind in BASE_SERVICES:
        if kind not in saved:
            legacy = await store.get_setting(f"provider.{kind}")
            if legacy:
                saved[kind] = legacy
    for kind, base in BASE_SERVICES.items():
        merged = dict(base)
        merged.update({k: v for k, v in (saved.get(kind) or {}).items() if k in base})
        SERVICES[kind] = merged
    await save_services()


async def save_services() -> None:
    await store.set_setting("services", {k: dict(v) for k, v in SERVICES.items()})
...
async def force_join_ok(bot, uid: int) -> bool:
    if not settings.force_join_channel or is_admin(uid):
        return True
    if time.monotonic() - _join_cache.get(uid, -1e9) < 300:
        return True
    try:
        member = await bot.get_chat_member(f"@{settings.force_join_channel}", uid)
        ok = member.status not in ("left", "kicked")
    except TelegramError as exc:
        log.warning("Force-join check failed (%s) — allowing user %s", exc, uid)
        ok = True
    if ok:
...
class SMMProvider:
    ...
    async def add(self, cfg: dict, link: str, quantity: int) -> str:
        data = await self.call(cfg, action="add", service=cfg["service_id"], link=link, quantity=quantity)
...
async def auto_place(order: dict) -> tuple[bool, str | None]:
    ...
    if not provider_ready(spec):
        return False, "Auto-placement not configured — manual fulfilment."
    claimed = await store.transition_order(order["order_id"], ["pending"], "placing")
    if not claimed:
        return False, "Order is no longer pending."
    try:
        pid = await provider.add(spec, order["link"], int(order["quantity"]))
    ...
    await store.transition_order(order["order_id"], ["placing"], "processing", provider_order_id=pid,
                                 provider_error=None, provider_url=spec["api_url"], provider_service=spec["service_id"],
                                 placed_at=utcnow())
    return True, None
...
async def apply_provider_status(bot, order: dict, info: dict) -> str | None:
    ...
    label = service(order["kind"])["label"] if order["kind"] in SERVICES else order["kind"]
    if status == "completed":
...
def _cfg_for_order(order: dict) -> dict | None:
    ...
    for spec in SERVICES.values():
        if url and spec.get("api_url") == url and spec.get("api_key"):
            return spec
    spec = SERVICES.get(order["kind"])
    return spec if spec and spec.get("api_key") else None
...
def home_screen(user: dict, admin: bool) -> Screen:
    likes = service("likes")
    text = (
        f"{e('SKULL')} <b>{fancy('Welcome To')} {esc(settings.bot_name)}</b> {e('SKULL')}\n{HR}\n\n"
        f"{e('FIRE')} <b>{fancy('The Premium Instagram Growth Panel')}</b>\n\n"
        f"{e('HEART')} {fancy('Likes')}  ·  {e('EYE')} {fancy('Views')}  ·  {e('GEM')} {fancy('Coin Wallet')}\n\n"
    ...
    rows = [
        [btn(f"IG Likes · {fmt_num(likes['rate_per_1k'])}/K", "svc:likes", icon="HEART", style="danger"),
         btn("IG Views", "views", icon="EYE", style="primary")],
...
def views_screen() -> Screen:
    lines = "\n".join(f"{e(service(k)['emoji_key'])} {esc(service(k)['short'])}: <b>{fmt_num(service(k)['rate_per_1k'])} coins / 1K</b>"
                      + ("" if service(k).get("enabled", True) else " <i>(paused)</i>") for k in VIEW_KINDS)
    spec = service(VIEW_KINDS[0])
    text = title("EYE", "Instagram Views") + f"Choose the delivery speed:\n\n{lines}\n\n{e('TARGET')} Range: <b>{spec['min']:,} – {spec['max']:,}</b> views"
    rows = [[btn(svc_label(k), f"svc:{k}", icon=service(k)["emoji_key"], style=service(k)["style"])] for k in VIEW_KINDS]
...
def pick_service_screen(link: str) -> Screen:
    ...
    rows = [[btn(svc_label(k), f"svc:{k}", icon=service(k)["emoji_key"], style=service(k)["style"])] for k in SERVICES]
    return Screen(text, kb(*rows, cancel_btn()))


def link_prompt(kind: str, error: str | None = None) -> Screen:
    spec = service(kind)
    text = (title(spec["emoji_key"], spec["label"]) +
...
def qty_screen(kind: str, link: str, error: str | None = None) -> Screen:
    ...
    text = (title(spec["emoji_key"], spec["label"]) +
            ...
            f"Allowed: <b>{spec['min']:,} – {spec['max']:,}</b> · step <b>{spec['step']:,}</b>\n"
...
def review_screen(draft: dict, balance: int, note: str | None = None) -> Screen:
    ...
    text = (title("REPORT", "Review Your Order") +
            ...
            f"{e('QUANTITY')} Quantity: <b>{qty:,} {spec['unit']}</b>\n"
    ...
    if note:
        text += f"\n\n{e('INFO')} {esc(note)}"
    rows_rich = [("Service", esc(spec["label"])), ("Quantity", f"{qty:,} {spec['unit']}"), ("Cost", f"{coins} coins"),
    ...
    if enough:
        first = [btn(f"Confirm & Pay {coins}", "ord:ok", icon="CHECK", style="success")]
    else:
        first = [btn("Deposit Coins", "dep", icon="DEPOSIT", style="success")]
    return Screen(text, kb(first, [btn("Quantity", "ord:qty", icon="EDIT"), btn("Link", "ord:link", icon="LINK")],
                           cancel_btn()), rich)
...
def receipt_screen(order: dict, error: str | None = None) -> Screen:
    ...
    status_line = (f"{e('ROCKET')} <b>Live on the network</b> — delivery has started."
                   if placed else f"{e('LOADING')} <b>Queued</b> — our team will process it shortly.")
...
def orders_screen(rows: list[dict], total: int, page: int) -> Screen:
    if not rows:
        return Screen(title("ORDER", "My Orders") + "No orders yet. Choose Likes or Views from the menu to start.",
                      kb([btn("IG Likes", "svc:likes", icon="HEART", style="danger"), btn("IG Views", "views", icon="EYE", style="primary")], home_btn()))
    pages = max(1, math.ceil(total / PAGE))
    lines, buttons = [], []
    for o in rows:
        spec = SERVICES.get(o["kind"], {"short": o["kind"], "emoji_key": "SERVICE"})
...
def _order_rows(o: dict) -> list[tuple[str, str]]:
    spec = SERVICES.get(o["kind"], {"label": o["kind"]})
...
def order_detail_screen(o: dict, note: str | None = None) -> Screen:
    ...
    if o["status"] == "processing":
        rows.append([btn("Refresh Status", f"od:r:{o['order_id']}", icon="REFRESH", style="primary")])
...
def deposit_menu_screen() -> Screen:
    text = title("DEPOSIT", "Deposit Coins") + f"{esc(settings.deposit_info)}\n\n{e('DOWN')} Pick a package:"
    rows = [[btn(f"{coins:,} coins · {settings.currency}{price:,}", f"dep:p:{i}", icon="COINS")]
            for i, (coins, price) in enumerate(settings.coin_packages)]
...
def deposit_pay_screen(dep: dict, error: str | None = None) -> Screen:
    ...
    text = (title("DEPOSIT", f"Deposit #{dep['deposit_id']}") +
            f"{e('COINS')} Package: <b>{dep['coins']:,} coins</b>\n{e('MONEY')} Pay: <b>{settings.currency}{dep['price']:,}</b>\n"
            + (f"{e('UPI')} UPI: <code>{esc(settings.upi_id)}</code>\n" if settings.upi_id else "")
            + f"\n{esc(settings.deposit_info)}\n\n"
    ...
    rows = []
    if settings.upi_id:
        rows.append([btn("Copy UPI ID", copy=settings.upi_id, icon="COPY"), btn("Copy Amount", copy=str(dep["price"]), icon="COPY")])
...
def referral_screen(user: dict, link: str) -> Screen:
    share = f"https://t.me/share/url?url={quote(link, safe='')}&text={quote('Grow your Instagram with ' + settings.bot_name)}"
    text = (title("REFER", "Referral Program") +
            f"Invite friends and earn <b>+{settings.referral_bonus} coins</b> for every new user who joins with your link.\n\n"
...
def bonus_screen(claimed: bool, user: dict | None, next_at: datetime | None) -> Screen:
    if claimed:
        text = (title("BONUS", "Daily Bonus Claimed") + f"{e('GEM')} <b>+{settings.daily_bonus} coins</b> added!\n"
                f"New balance: <b>{fmt_num((user or {}).get('coins', 0))}</b>\n\nCome back in {settings.daily_cooldown_hours}h for more.")
    else:
        left = fmt_left(next_at - utcnow()) if next_at else f"{settings.daily_cooldown_hours}h"
...
def support_screen() -> Screen:
    text = title("SUPPORT", "Support") + f"Need help with an order or a deposit? Contact the operator:\n\n{esc(settings.support_url)}"
    return Screen(text, kb([btn("Open Support", url=settings.support_url, icon="SUPPORT", style="primary")], home_btn()))


def join_screen() -> Screen:
    text = title("JOIN", "Join Our Channel") + "Please join our channel to use this bot, then tap <b>I've Joined</b>."
    return Screen(text, kb([btn("Join Channel", url=f"https://t.me/{settings.force_join_channel}", icon="JOIN", style="primary")],
                           [btn("I've Joined", "join", icon="CHECK", style="success")]))
...
def banned_screen() -> Screen:
    return Screen(title("BAN", "Access Restricted") + "You are banned from using this bot. Contact support if this is a mistake.",
                  kb([btn("Support", url=settings.support_url, icon="SUPPORT")]))
...
def admin_screen(st: dict, maintenance: bool) -> Screen:
    ...
    return Screen(text, kb(
        ...
        [btn("Services", "a:sv", icon="SETTING"), btn("Find User", "a:find", icon="SEARCH")],
        [btn("Redeem Codes", "a:rc", icon="REDEEM"), btn("Broadcast", "a:bc", icon="BROADCAST")],
...
def admin_stats_screen(st: dict) -> Screen:
    rows = [("Total users", f"{st['users']:,}"), ("New today", str(st["users_today"])), ("Banned", str(st["banned"])),
            ...
            ("Coins spent", fmt_num(st["coins_spent"])), ("Deposit revenue", f"{settings.currency}{fmt_num(st['revenue'])}")]
...
def admin_queue_screen(status: str, rows: list[dict]) -> Screen:
    ...
    lines = [f"<code>#{o['order_id']}</code> · {esc(SERVICES.get(o['kind'], {}).get('short', o['kind']))} · {o['quantity']:,} · "
...
def admin_order_text(o: dict, user: dict | None, note: str | None = None) -> str:
    spec = SERVICES.get(o["kind"], {"label": o["kind"], "service_id": "?"})
    text = (title(ORDER_STATUS_ICON.get(o["status"], "ORDER"), f"Order #{o['order_id']}") +
            ...
            f"<b>Service:</b> {esc(spec['label'])} · ID <code>{esc(spec.get('service_id', ''))}</code>\n"
            f"<b>Quantity:</b> {o['quantity']:,} · <b>Coins:</b> {o['coins']}\n"
            f"<b>Status:</b> {esc(o['status'].title())}"
            + (f" · provider <code>{esc(o['provider_order_id'])}</code>" if o.get("provider_order_id") else "")
...
def admin_deposits_screen(rows: list[dict]) -> Screen:
    ...
    lines = [f"<code>#{d['deposit_id']}</code> · <code>{d['user_id']}</code> · {d['coins']:,} coins · {settings.currency}{d['price']:,}"
             f" · <i>{d['status'].replace('_', ' ')}</i>" for d in rows]
    buttons = [[btn(f"#{d['deposit_id']} · {settings.currency}{d['price']:,}", f"a:d:{d['deposit_id']}", icon="DEPOSIT")] for d in rows[:12]]
...
def deposit_admin_text(d: dict, user: dict | None) -> str:
    return (title("DEPOSIT", f"Deposit #{d['deposit_id']}") +
            ...
            f"\n<b>Package:</b> {d['coins']:,} coins for {settings.currency}{d['price']:,}\n"
...
def admin_services_screen() -> Screen:
    lines = []
    for kind, s in SERVICES.items():
        auto = "auto" if provider_ready(s) else "manual"
        lines.append(f"{e(s['emoji_key'])} <b>{esc(s['label'])}</b> · {fmt_num(s['rate_per_1k'])}/K · "
                     f"{'🟢 on' if s.get('enabled', True) else '🔴 off'} · {auto}")
    rows = [[btn(svc_label(k), f"a:sv:{k}", icon=SERVICES[k]["emoji_key"], style=SERVICES[k]["style"])] for k in SERVICES]
    return Screen(title("SETTING", "Services") + "\n".join(lines), kb(*rows, [btn("Back", "a", icon="BACK", style="danger")]))


def admin_service_screen(kind: str, note: str | None = None) -> Screen:
    s = service(kind)
    key = s.get("api_key", "")
    masked = f"••••{key[-4:]}" if len(key) > 6 else ("set" if key else "not set")
    text = (title(s["emoji_key"], s["label"]) +
            f"<b>Status:</b> {'🟢 Enabled' if s.get('enabled', True) else '🔴 Disabled'} · "
            f"{'⚡ Auto-placement' if provider_ready(s) else '✋ Manual'}\n"
            f"<b>API URL:</b> <code>{esc(s['api_url'])}</code>\n<b>API key:</b> <code>{masked}</code>\n"
            f"<b>Service ID:</b> <code>{esc(s['service_id'] or '—')}</code>\n"
            f"<b>Rate:</b> {fmt_num(s['rate_per_1k'])} coins / 1K\n<b>Range:</b> {s['min']:,} – {s['max']:,} · step {s['step']:,}")
    if note:
        text += f"\n\n{e('INFO')} {esc(note)}"
    return Screen(text, kb(
        [btn("Name", f"a:sf:label:{kind}", icon="EDIT"), btn("API URL", f"a:sf:url:{kind}", icon="LINK")],
        [btn("API Key", f"a:sf:key:{kind}", icon="KEY"), btn("Service ID", f"a:sf:sid:{kind}", icon="ID")],
        [btn("Rate /1K", f"a:sf:rate:{kind}", icon="COINS"), btn("Step", f"a:sf:step:{kind}", icon="QUANTITY")],
        [btn("Min", f"a:sf:min:{kind}", icon="TARGET"), btn("Max", f"a:sf:max:{kind}", icon="TARGET")],
        [btn("Disable" if s.get("enabled", True) else "Enable", f"a:sx:{kind}", icon="POWER",
             style="danger" if s.get("enabled", True) else "success"),
         btn("Provider Balance", f"a:sb:{kind}", icon="MONEY")],
...
def admin_field_prompt(kind: str, field: str, error: str | None = None) -> Screen:
    ...
    current = "(hidden)" if ftype == "secret" else esc(s.get(storage, ""))
    text = title("EDIT", f"Edit {label}") + f"Service: <b>{esc(s['label'])}</b>\nCurrent: <code>{current}</code>\n\nSend the new value."
    if ftype == "secret":
        text += f"\n{e('LOCK')} Your message is deleted immediately."
    if error:
        text += f"\n\n{e('ERROR')} <b>{esc(error)}</b>"
    return Screen(text, kb(cancel_btn(f"a:sv:{kind}")))
...
async def on_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not admin_only:
        ...
        if user.get("banned") and not is_admin(uid):
            ...
            return
        if fn is not cb_join and not await force_join_ok(context.bot, uid):
            await render(update, context, join_screen())
...
async def cb_join(update, context, args):
    ...
    if not await force_join_ok(context.bot, update.effective_user.id):
        return "You haven't joined yet.", True
    await go_home(update, context)
    return "✅ Welcome!"


@route("views")
async def cb_views(update, context, args):
    await render(update, context, views_screen())
...
async def start_order(update, context, kind: str, link: str | None = None) -> Result:
    ...
    if await maintenance_on() and not is_admin(update.effective_user.id):
        ...
        return None
    if not service(kind).get("enabled", True):
    ...
    if link:
        ...
        await render(update, context, qty_screen(kind, link))
    else:
...
async def show_review(update, context, qty: int, note: str | None = None) -> None:
    ...
    set_await(context, "order_qty")  # typing another number simply updates the quantity
...
async def cb_order(update, context, args):
    ...
    elif action == "qty" and "link" in draft:
        set_await(context, "order_qty")
        await render(update, context, qty_screen(draft["kind"], draft["link"]))
    elif action == "ok":
...
async def confirm_order(update, context) -> Result:
    ...
    async with lock:
        ...
        if not service(kind).get("enabled", True):
            return "⏸ This service is paused right now.", True
        if err := check_quantity(kind, draft["quantity"]):
            await render(update, context, qty_screen(kind, draft["link"], err))
            return None
        ...
        try:
            order = await store.create_order(oid, uid, kind, draft["link"], draft["quantity"], fresh_price)
        except Exception:
...
async def place_order_flow(app: Application, order: dict, uid: int, chat_id: int, panel_id: int, seq: int) -> None:
    ...
    note = f"{e('CHECK')} Auto-placed on provider (<code>{esc(fresh.get('provider_order_id'))}</code>)." if placed \
        else f"{e('WARN')} Needs manual action: <i>{esc(error)}</i>"
...
async def cb_order_detail(update, context, args):
    refresh = bool(args) and args[0] == "r"
    oid = args[1] if refresh and len(args) > 1 else (args[0] if args else "")
    order = await store.get_order(oid)
    if not order or (order["user_id"] != update.effective_user.id and not is_admin(update.effective_user.id)):
        return "Order not found.", True
    note = None
    if refresh and order["status"] == "processing":
    ...
    return "🔄 Updated" if refresh else None
...
async def cb_deposit(update, context, args):
    ...
    if args and args[0] == "p" and len(args) > 1 and args[1].isdigit():
        idx = int(args[1])
        if idx >= len(settings.coin_packages):
            return "Package not available.", True
        coins, price = settings.coin_packages[idx]
        dep = await store.create_deposit(uid, coins, price, settings.deposit_expiry_minutes)
...
async def cb_bonus(update, context, args):
    claimed, user, next_at = await store.claim_daily(update.effective_user.id, settings.daily_bonus, settings.daily_cooldown_hours)
    await render(update, context, bonus_screen(claimed, user, next_at))
    return f"🎁 +{settings.daily_bonus} coins!" if claimed else None
...
async def cb_admin_services(update, context, args):
    clear_await(context)
    if args and args[0] in SERVICES:
        await render(update, context, admin_service_screen(args[0]))
    else:
...
async def cb_admin_service_field(update, context, args):
    if len(args) < 2 or args[0] not in EDITABLE_FIELDS or args[1] not in SERVICES:
        return None
    set_await(context, "svc_field", field=args[0], service=args[1], admin=True)
...
async def cb_admin_service_toggle(update, context, args):
    ...
    await save_services()
    await render(update, context, admin_service_screen(kind))
    return "🟢 Enabled" if SERVICES[kind]["enabled"] else "🔴 Disabled"
...
async def cb_admin_service_balance(update, context, args):
    ...
    if kind not in SERVICES:
        return None
    try:
        return f"💰 Provider balance: {await provider.balance(SERVICES[kind])}", True
    except ProviderError as exc:
        return f"❌ {exc}", True
...
async def on_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    secret = bool(state and state.get("kind") == "svc_field" and EDITABLE_FIELDS.get(state.get("field"), ("", "", ""))[2] == "secret")
    ...
    if user.get("banned") and not is_admin(uid):
        ...
        return
    if not await force_join_ok(context.bot, uid):
        await render(update, context, join_screen())
        return
...
async def in_service_field(update, context, state, text):
    ...
    await save_services()
    clear_await(context)
    await render(update, context, admin_service_screen(kind, f"{label} updated."))
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if is_new and context.args:
        ...
        if raw.isdigit() and await store.claim_referral(uid, int(raw), settings.referral_bonus):
            await notify(context.bot, int(raw), f"{e('REFERRAL')} <b>{fancy('Referral Confirmed')}</b>\n{HR}\n\n"
                                                f"{e('GEM')} <b>+{settings.referral_bonus} coins</b> — "
                                                f"{esc(update.effective_user.first_name)} joined with your link!")
    if not await force_join_ok(context.bot, uid):
        await render(update, context, join_screen(), force_new=True)
        return
...
async def post_init(app: Application) -> None:
    ...
    log.info("%s ready as @%s · storage=%s · rich=%s · drafts=%s", settings.bot_name, app.bot.username,
